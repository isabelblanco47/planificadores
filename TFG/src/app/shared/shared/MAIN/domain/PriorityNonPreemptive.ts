...
export class PriorityNonPreemptive implements SchedulingAlgorithm {
  ...
  startTask(task: TaskControlBlock): void {
    ...
    if (this.next_state.running_task === null) {
      ...
      this.ready_task_queue.insertSorted(task, PriorityNonPreemptive.sortByPriority);
    }
  ...
  yieldHardDisk(): void {
    ...
    if (this.getHardDiskTask() === null) {
      ...
      this.hard_disk_waiting_queue.insertSorted(running_task, PriorityNonPreemptive.sortByPriority);
    }
  ...
  ioHardDiskIRQ(): void {
    ...
    if (this.next_state.running_task !== null) {
      ...
      this.ready_task_queue.insertSorted(hard_disk_task, PriorityNonPreemptive.sortByPriority);
    } else {
  ...
  yieldKeyboard(): void {
    ...
    if (this.getKeyboardTask() === null) {
      ...
      this.keyboard_waiting_queue.insertSorted(running_task, PriorityNonPreemptive.sortByPriority);
    }
  ...
  ioKeyboardIRQ(): void {
    ...
    if (this.next_state.running_task !== null) {
      ...
      this.ready_task_queue.insertSorted(keyboard_task, PriorityNonPreemptive.sortByPriority);
    } else {
      // If there was no task previously going to run, set this one to running
      // state and dispatch it to the CPU
